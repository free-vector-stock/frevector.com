...
/* --- OUR SELECTIONS FONKSİYONEL REVİZYON --- */
(function() {
    const picksContainer = document.querySelector('.our-picks-scroll');
    if (!picksContainer) return;

    // 1. Okları DOM'a ekle
    const wrapper = picksContainer.parentElement;
    wrapper.classList.add('our-picks-wrapper');
    
    const prevBtn = document.createElement('button');
    const nextBtn = document.createElement('button');
    prevBtn.className = 'pick-arrow prev';
    nextBtn.className = 'pick-arrow next';
    prevBtn.innerHTML = '&#10094;';
    nextBtn.innerHTML = '&#10095;';
    ...
    // 2. Kategori Tıklandığında Karıştırma Mantığı
    window.refreshOurPicks = function(categoryData) {
        let allImgs = [];
        // Mevcut projedeki data yapısına göre görselleri topla
        if (categoryData && categoryData.pages) {
            categoryData.pages.forEach(p => {
                if(p.images) allImgs = allImgs.concat(p.images);
            });
        }
        ...
        // İçeriği temizle ve bas
        picksContainer.innerHTML = '';
        allImgs.forEach(imgData => {
            const img = document.createElement('img');
            img.src = imgData.thumb || imgData; // thumb varsa onu kullan
            img.className = 'pick-item';
            picksContainer.appendChild(img);
        });

        // Sonsuz döngü için içeriği çiftle
        const clone = picksContainer.innerHTML;
        picksContainer.innerHTML += clone;
    };

    // 3. Sonsuz Kaydırma ve Ok Tıklama Kontrolü
    const step = 320;
    nextBtn.onclick = () => {
        picksContainer.scrollLeft += step;
        // Yarıyı geçtiyse (klon kısma geldiyse) başa at
        if (picksContainer.scrollLeft >= picksContainer.scrollWidth / 2) {
            picksContainer.scrollLeft = 0;
        }
    };

    prevBtn.onclick = () => {
        if (picksContainer.scrollLeft <= 0) {
            picksContainer.scrollLeft = picksContainer.scrollWidth / 2;
        }
        picksContainer.scrollLeft -= step;
    };
})();
